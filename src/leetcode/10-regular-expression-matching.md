...
$$
f[i][j]=\begin{cases}
if (p[j] ≠ *)=\begin{cases} f[i-1][j−1], matches(s[i],p[j]) \\ false, otherwise \end{cases} \\
otherwise=\begin{cases} f[i−1][j] or f[i][j−2], matches(s[i],p[j-1]) \\ f[i][j−2], otherwise \end{cases}
\end{cases}
$$
